...
filters = [np.random.normal(size=(4, 4)) for _ in range(num_workers)]

def run_benchmark():
    image = np.zeros((10000, 5000))
    image_id = ray.put(image)
    ray.get([f.remote(image_id, filters[i]) for i in range(num_workers)])

def run_benchmark_new(image_id):
    ray.get([f.remote(image_id, filters[i]) for i in range(num_workers)])
...
durations2 = []
for _ in range(num_trials):
    streaming_actors = [StreamingPrefixCount.remote() for _ in range(num_workers)]

    start_time = time.time()

    for i in range(num_workers* 10):
        document = [np.random.bytes(20) for _ in range(10000)]
        streaming_actors[i % num_workers].add_document.remote(document)
...
actors = [Model.remote(i) for i in range(num_workers)]
